...
const Vorstand = () => {
    ...
    useEffect(() => {
        const query = `*[_type == "vorstand"] | order(name asc) {
            _id,
            name,
            mail,
            funktionen,
        ...
        <div className="bg-white/80 rounded-2xl shadow-xl border border-[#e0e7ff] px-2 sm:px-6 py-6 sm:py-8 w-full">
            <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
                {mitglieder.map((mitglied) => {
                    const imageUrl = getImageUrl(mitglied.bild, 400, 300);
                    return (
                        <div
                            key={mitglied._id}
                        ...
                        >
                            ...
                            <div className="p-2 sm:p-4 text-center">
                                ...
                                <ul className="text-xs sm:text-sm text-gray-600 mt-1 space-y-1">
                                    {mitglied.funktionen?.map((fkt, idx) => (
                                        <li key={idx}>{fkt}</li>
                                    ))}
                                </ul>
                                <div className="text-xs text-gray-500 mt-2">{mitglied.mail}</div>
                            </div>
            ...
            {selected && (
                ...
                >
                    ...
                    <div className="text-center">
                        ...
                        <div className="text-xs text-gray-500 mb-4">{selected.mail}</div>
